...
const sendSingleCityData = (response, language, locale) => {
  ...
  *-------- COVID-19 --------*
  ...
  ${language.confirmedLabel} *${response.confirmed}*
  ${language.deathsLabel} *${response.deaths}*
  ${language.estimatedPopulation} *${response.estimated_population_2019}*
...
const sendBrazilData = (confirmed, deaths, date, language, locale) => {
  ...
  *-------- COVID-19 --------*
  ...
  ${language.confirmedLabel} *${confirmed}*
  ${language.deathsLabel} *${deaths}*
  ${language.infoProvidedBy} *${language.ministryOfHealthAndWho}*
...
client.on('message', async (msg) => {
  ...
  const fetchCountryData = async (param) => {
    try {
      ...
      console.error(error, 'Error on fetchCountryData');
    }
  };
    ...
    if (msg.body.startsWith('!')) {
      if (countriesArray) {
        ...
        const placeToUpperCase =
          placeOnly[0].toUpperCase() + placeOnly.substring(1);
        fetchGeneralData(`city=${placeToUpperCase}`, true);
